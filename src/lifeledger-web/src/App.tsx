...
import { assetKindLabel } from './assetCategories'
...
type Page = 'dashboard' | 'wealth' | 'planner' | 'banking' | 'simulator' | 'scenarios' | 'compare' | 'settings'
...
export function App() {
  ...
  const [theme, setTheme] = useState<Theme>(() => localStorage.getItem('lifeledger.theme') === 'light' ? 'light' : 'dark')
  ...
  function changeTheme(nextTheme: Theme) {
    setTheme(nextTheme)
    localStorage.setItem('lifeledger.theme', nextTheme)
  }
  ...
  async function openSettings() {
    try {
      ...
      setProfile(nextProfile); setAssetCategories(nextAssetCategories); setAssetProfileDefinitions(nextAssetProfileDefinitions); setPage('settings')
    }
  ...
  async function restoreBackup(document: LifeLedgerExport) {
    try { await api.importData(document, true); await refresh(); setPage('dashboard') }
      ...
      <div className="mx-auto grid min-h-[calc(100vh-1.5rem)] max-w-[1560px] lg:grid-cols-[244px_1fr]">
        <aside className="glass mb-4 flex rounded-3xl p-3 lg:mb-0 lg:flex-col lg:rounded-r-none lg:border-r-0 lg:p-5">
          ...
          <nav className="ml-auto flex gap-1 overflow-x-auto lg:ml-0 lg:mt-12 lg:block lg:space-y-1" aria-label="Primary navigation">
            ...
            <button className={`nav-item shrink-0 lg:hidden ${page === 'settings' ? 'nav-item-active' : ''}`} onClick={() => void openSettings()}><span className="text-base">⚙</span><span className="hidden sm:inline">{tr(locale, 'Settings', 'Paramètres')}</span></button>
          </nav>
          <div className="mt-auto hidden space-y-3 lg:block"><div className="glass rounded-2xl p-4"><p className="eyebrow">{text.privateData}</p><p className="mt-2 text-sm font-medium text-white">{text.privateDetail}</p><p className="mt-1 text-xs leading-5 text-muted">{tr(locale, 'Saved locally in your own database. No accounts, trackers or remote calls.', 'Enregistrées localement dans votre base. Aucun compte, suivi ou appel distant.')}</p></div><button className={`nav-item ${page === 'settings' ? 'nav-item-active' : ''}`} onClick={() => void openSettings()}><span className="text-base">⚙</span><span>{tr(locale, 'Settings', 'Paramètres')}</span></button></div>
        </aside>
...
function DashboardPage({ allocationStrategies, dashboard, locale, onPlan, onSaveStrategy, onDeleteStrategy }: { allocationStrategies: AllocationStrategy[]; dashboard: Dashboard; locale: Locale; onPlan: () => void; onSaveStrategy: (strategyId: string | undefined, value: Record<string, unknown>) => Promise<void>; onDeleteStrategy: (strategyId: string) => Promise<void> }) {
  ...
  return <section className="space-y-5"><div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="eyebrow">{tr(locale, 'Long-term clarity', 'Vision à long terme')}</p><h1 className="mt-2 max-w-2xl text-3xl font-semibold tracking-tight text-white sm:text-4xl">{tr(locale, 'See where your current life could lead.', 'Découvrez où votre mode de vie actuel peut vous mener.')}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted">{tr(locale, 'A living model of your income, assets, debt, expenses and life events—across the next fifty years.', 'Un modèle vivant de vos revenus, actifs, dettes, dépenses et événements de vie sur les cinquante années à venir.')}</p></div><button className="ghost-button" onClick={onPlan}>{tr(locale, 'Edit life inputs', 'Modifier les données')}</button></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><MetricCard label={tr(locale, 'Current net worth', 'Patrimoine net actuel')} value={formatMoney(dashboard.currentNetWorth, dashboard.currency, true, locale)} detail={tr(locale, 'Assets less liabilities today', 'Actifs moins dettes aujourd’hui')} icon="◈" /><MetricCard label={tr(locale, '10-year net worth', 'Patrimoine net à 10 ans')} value={formatMoney(inTenYears.netWorth, dashboard.currency, true, locale)} detail={`${tr(locale, 'At age', 'À')} ${inTenYears.age} {tr(locale, 'years', 'ans')}`} icon="↗" tone="success" /><MetricCard label={tr(locale, 'Passive cash received', 'Revenu passif encaissé')} value={formatMoney(dashboard.passiveMonthlyIncome, dashboard.currency, true, locale)} detail={tr(locale, 'Rent, dividends and royalties—not asset appreciation', 'Loyers, dividendes et redevances — hors hausse des actifs')} icon="⌁" /><MetricCard label={tr(locale, 'Estimated portfolio growth', 'Croissance estimée du portefeuille')} value={formatMoney(dashboard.expectedMonthlyPortfolioGrowth, dashboard.currency, true, locale)} detail={tr(locale, 'Average monthly appreciation—not cash received', 'Hausse mensuelle moyenne — non encaissée')} icon="↗" tone="success" /><MetricCard label={tr(locale, 'Success probability', 'Probabilité de réussite')} value={`${Math.round(dashboard.probabilityOfSuccess * 100)}%`} detail={tr(locale, 'Monte Carlo solvency rate', 'Taux de solvabilité Monte Carlo')} icon="◌" tone={dashboard.probabilityOfSuccess >= .8 ? 'success' : 'warning'} /></div><div className="grid gap-4 xl:grid-cols-[1.65fr_1fr]"><article className="section-card"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">{tr(locale, 'Wealth timeline', 'Évolution du patrimoine')}</p><h2 className="mt-1 text-lg font-semibold text-white">{tr(locale, 'Total wealth and value by category', 'Patrimoine total et détail par catégorie')}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">{tr(locale, 'Each coloured area follows what you own. Property appreciation increases wealth without becoming cash income; its projected sale value is already included in the total line.', 'Chaque zone colorée suit ce que vous possédez. La hausse d’un bien immobilier augmente votre patrimoine sans devenir un revenu encaissé ; sa valeur de revente projetée est déjà comprise dans la ligne totale.')}</p></div><div className="flex items-center gap-3"><span className="rounded-full bg-sky/15 px-3 py-1 text-xs font-medium text-sky">{tr(locale, 'to age', 'jusqu’à')} {dashboard.timeline.at(-1)?.age} {tr(locale, 'years', 'ans')}</span>{scaleControl}</div></div><div className="mt-5"><NetWorthChart timeline={dashboard.timeline} currency={dashboard.currency} locale={locale} scale={timelineScale} /></div></article><article className="section-card"><p className="eyebrow">{tr(locale, 'Financial independence', 'Indépendance financière')}</p><p className="mt-2 text-2xl font-semibold text-white">{formatDate(dashboard.financialIndependenceDate, locale)}</p><p className="mt-2 text-sm leading-6 text-muted">{tr(locale, 'The first point where your projected wealth can support planned annual spending at your safe withdrawal rate.', 'Le premier moment où votre patrimoine projeté peut financer vos dépenses annuelles prévues selon votre taux de retrait sûr.')}</p><dl className="mt-6 space-y-3 border-t border-white/10 pt-5 text-sm"><div className="flex justify-between gap-4"><dt className="text-muted">{tr(locale, 'Retirement income', 'Revenu à la retraite')}</dt><dd className="font-medium text-mist">{formatMoney(dashboard.estimatedRetirementIncome, dashboard.currency, false, locale)}/{tr(locale, 'mo', 'mois')}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted">{tr(locale, 'Real wealth change', 'Évolution réelle du patrimoine')}</dt><dd className={dashboard.inflationAdjustedPurchasingPowerChange >= 0 ? 'font-medium text-success' : 'font-medium text-warning'}>{dashboard.inflationAdjustedPurchasingPowerChange >= 0 ? '+' : ''}{Math.round(dashboard.inflationAdjustedPurchasingPowerChange)}%</dd></div></dl></article></div><div className="grid gap-4 xl:grid-cols-2"><article className="section-card"><p className="eyebrow">{tr(locale, 'Portfolio allocation', 'Répartition du portefeuille')}</p><h2 className="mt-1 text-lg font-semibold text-white">{tr(locale, 'What you own today', 'Ce que vous possédez aujourd’hui')}</h2><div className="mt-4"><AllocationChart allocation={dashboard.allocation} currency={dashboard.currency} /></div></article><AllocationStrategyCard allocation={dashboard.allocation} assessment={dashboard.allocationStrategy} currency={dashboard.currency} locale={locale} strategies={allocationStrategies} onDelete={onDeleteStrategy} onSave={onSaveStrategy} /></div><article className="section-card"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="eyebrow">{tr(locale, 'Cash flow timeline', 'Évolution de la trésorerie')}</p><h2 className="mt-1 text-lg font-semibold text-white">{tr(locale, 'Annual surplus after planned saving', 'Excédent annuel après épargne prévue')}</h2></div>{scaleControl}</div><div className="mt-4"><CashFlowChart timeline={dashboard.timeline} currency={dashboard.currency} locale={locale} scale={timelineScale} /></div></article><Warnings items={dashboard.warnings} locale={locale} /></section>
}

const allocationStrategyCopy: Record<Locale, Record<string, string>> = {
  en: { eyebrow: 'Target allocation', none: 'No active strategy', manage: 'Manage', allInside: 'Every category is inside its acceptable range.', outside: 'categories outside their acceptable range.', current: 'Current share', currentValue: 'Current value', portfolioValue: 'Current portfolio value', portfolioValueHelp: 'Total value of the asset categories used by this strategy.', target: 'Your target', acceptable: 'Acceptable range', difference: 'Difference from target', intro: 'Set the share you would ideally like for each category. LifeLedger only warns you when the current share leaves the acceptable range; it never buys or sells automatically.', strategyName: 'Strategy name', effectiveFrom: 'Effective from', endDate: 'End date', rationale: 'Rationale (optional)', category: 'Category', targetInput: 'Desired share (%)', toleranceInput: 'Allowed difference (± points)', toleranceHelp: 'Example: a 70% target with 5 points allows a share between 65% and 75%.', defaultHelp: 'For a new strategy, the desired share starts with your current allocation. The proposed 5 is an allowed difference of 5 percentage points.', add: 'Add category', save: 'Save strategy', cancel: 'Cancel', delete: 'Delete version', remove: 'Remove' },
  fr: { eyebrow: 'Répartition souhaitée', none: 'Aucune stratégie active', manage: 'Gérer', allInside: 'Toutes les catégories sont dans leur plage acceptable.', outside: 'catégorie(s) hors de leur plage acceptable.', current: 'Part actuelle', currentValue: 'Valeur actuelle', portfolioValue: 'Valeur actuelle du portefeuille', portfolioValueHelp: 'Somme des catégories d’actifs prises en compte par cette stratégie.', target: 'Votre objectif', acceptable: 'Plage acceptable', difference: 'Écart avec l’objectif', intro: 'Définissez la part idéale de chaque catégorie. LifeLedger vous avertit seulement si la part actuelle sort de la plage acceptable ; il n’achète et ne vend jamais automatiquement.', strategyName: 'Nom de la stratégie', effectiveFrom: 'Valable à partir du', endDate: 'Date de fin', rationale: 'Raison (facultatif)', category: 'Catégorie', targetInput: 'Part souhaitée (%)', toleranceInput: 'Écart autorisé (± points)', toleranceHelp: 'Exemple : un objectif de 70 % avec 5 points autorise une part entre 65 % et 75 %.', defaultHelp: 'Pour une nouvelle stratégie, la part souhaitée reprend votre répartition actuelle. Le 5 proposé signifie un écart autorisé de 5 points de pourcentage.', add: 'Ajouter une catégorie', save: 'Enregistrer la stratégie', cancel: 'Annuler', delete: 'Supprimer la version', remove: 'Supprimer' },
  pl: { eyebrow: 'Docelowy podział', none: 'Brak aktywnej strategii', manage: 'Zarządzaj', allInside: 'Wszystkie kategorie mieszczą się w dopuszczalnym zakresie.', outside: 'kategorii poza dopuszczalnym zakresem.', current: 'Obecny udział', currentValue: 'Obecna wartość', portfolioValue: 'Obecna wartość portfela', portfolioValueHelp: 'Łączna wartość kategorii aktywów uwzględnionych w tej strategii.', target: 'Twój cel', acceptable: 'Dopuszczalny zakres', difference: 'Różnica względem celu', intro: 'Ustal idealny udział każdej kategorii. LifeLedger tylko ostrzega o wyjściu poza zakres i nigdy automatycznie nie kupuje ani nie sprzedaje.', strategyName: 'Nazwa strategii', effectiveFrom: 'Obowiązuje od', endDate: 'Data zakończenia', rationale: 'Uzasadnienie (opcjonalnie)', category: 'Kategoria', targetInput: 'Docelowy udział (%)', toleranceInput: 'Dopuszczalna różnica (± pkt)', toleranceHelp: 'Przykład: cel 70% i 5 punktów oznacza zakres od 65% do 75%.', defaultHelp: 'Nowa strategia zaczyna od obecnego podziału. Proponowane 5 oznacza 5 punktów procentowych dopuszczalnej różnicy.', add: 'Dodaj kategorię', save: 'Zapisz strategię', cancel: 'Anuluj', delete: 'Usuń wersję', remove: 'Usuń' },
  de: { eyebrow: 'Gewünschte Aufteilung', none: 'Keine aktive Strategie', manage: 'Verwalten', allInside: 'Alle Kategorien liegen im zulässigen Bereich.', outside: 'Kategorie(n) außerhalb des zulässigen Bereichs.', current: 'Aktueller Anteil', currentValue: 'Aktueller Wert', portfolioValue: 'Aktueller Portfoliowert', portfolioValueHelp: 'Gesamtwert der in dieser Strategie berücksichtigten Anlagekategorien.', target: 'Ihr Ziel', acceptable: 'Zulässiger Bereich', difference: 'Abweichung vom Ziel', intro: 'Legen Sie den idealen Anteil jeder Kategorie fest. LifeLedger warnt nur bei Abweichungen und kauft oder verkauft nie automatisch.', strategyName: 'Strategiename', effectiveFrom: 'Gültig ab', endDate: 'Enddatum', rationale: 'Begründung (optional)', category: 'Kategorie', targetInput: 'Gewünschter Anteil (%)', toleranceInput: 'Erlaubte Abweichung (± Punkte)', toleranceHelp: 'Beispiel: 70% Ziel mit 5 Punkten erlaubt einen Anteil zwischen 65% und 75%.', defaultHelp: 'Eine neue Strategie beginnt mit Ihrer aktuellen Aufteilung. Die vorgeschlagenen 5 bedeuten 5 Prozentpunkte erlaubte Abweichung.', add: 'Kategorie hinzufügen', save: 'Strategie speichern', cancel: 'Abbrechen', delete: 'Version löschen', remove: 'Entfernen' },
  nl: { eyebrow: 'Gewenste verdeling', none: 'Geen actieve strategie', manage: 'Beheren', allInside: 'Alle categorieën liggen binnen hun aanvaardbare bereik.', outside: 'categorie(ën) buiten hun aanvaardbare bereik.', current: 'Huidig aandeel', currentValue: 'Huidige waarde', portfolioValue: 'Huidige portefeuillewaarde', portfolioValueHelp: 'Totale waarde van de activacategorieën die deze strategie gebruikt.', target: 'Uw doel', acceptable: 'Aanvaardbaar bereik', difference: 'Verschil met het doel', intro: 'Bepaal het ideale aandeel per categorie. LifeLedger waarschuwt alleen bij afwijkingen en koopt of verkoopt nooit automatisch.', strategyName: 'Naam van strategie', effectiveFrom: 'Geldig vanaf', endDate: 'Einddatum', rationale: 'Reden (optioneel)', category: 'Categorie', targetInput: 'Gewenst aandeel (%)', toleranceInput: 'Toegestane afwijking (± punten)', toleranceHelp: 'Voorbeeld: een doel van 70% met 5 punten laat een aandeel tussen 65% en 75% toe.', defaultHelp: 'Een nieuwe strategie start met uw huidige verdeling. De voorgestelde 5 betekent 5 procentpunten toegestane afwijking.', add: 'Categorie toevoegen', save: 'Strategie opslaan', cancel: 'Annuleren', delete: 'Versie verwijderen', remove: 'Verwijderen' },
}

function AllocationStrategyCard({ allocation, assessment, currency, locale, strategies, onSave, onDelete }: { allocation: Dashboard['allocation']; assessment: Dashboard['allocationStrategy']; currency: string; locale: Locale; strategies: AllocationStrategy[]; onSave: (strategyId: string | undefined, value: Record<string, unknown>) => Promise<void>; onDelete: (strategyId: string) => Promise<void> }) {
  ...
  const copy = allocationStrategyCopy[locale]
  const formatPercent = (value: number) => `${new Intl.NumberFormat(locale, { maximumFractionDigits: 2 }).format(value)} %`
  const categoryOptions = Array.from(new Set([...allocation.map(slice => slice.name), ...targets.map(target => target.category).filter(Boolean)]))
  const portfolioValue = allocation.reduce((total, slice) => total + slice.value, 0)
  const currentAllocation = (category: string) => allocation.find(slice => slice.name.toLocaleLowerCase() === category.toLocaleLowerCase() || slice.kind.toLocaleLowerCase() === category.toLocaleLowerCase())

  return <article className="section-card">
    <div className="flex items-start justify-between gap-3"><div><p className="eyebrow">{copy.eyebrow}</p><h2 className="mt-1 text-lg font-semibold text-white">{assessment?.name ?? copy.none}</h2></div><button className="ghost-button px-3 py-2 text-xs" onClick={() => begin(assessment ? strategies.find(strategy => strategy.name === assessment.name && strategy.effectiveFrom === assessment.effectiveFrom) : undefined)}>{copy.manage}</button></div>
    <p className="mt-3 text-sm leading-6 text-muted">{copy.intro}</p>
    {assessment && <><p className={`mt-3 text-xs font-medium ${outOfRange ? 'text-warning' : 'text-success'}`}>{outOfRange ? `${outOfRange} ${copy.outside}` : copy.allInside}</p><div className="mt-4 space-y-3">{assessment.targets.map(target => {
      const minimum = Math.max(0, target.targetPercentage - target.tolerancePercentage)
      const maximum = Math.min(100, target.targetPercentage + target.tolerancePercentage)
      return <div className="rounded-2xl border border-white/10 bg-white/5 p-4" key={target.category}><div className="flex items-start justify-between gap-3"><span className="font-medium text-mist">{assetKindLabel(locale, target.category)}</span><span className={`rounded-xl px-2.5 py-1 text-xs font-semibold ${target.state === 'WithinRange' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>{copy.current}: {formatPercent(target.actualPercentage)}</span></div><dl className="mt-3 grid gap-2 border-t border-white/10 pt-3 text-xs sm:grid-cols-2"><div><dt className="text-muted">{copy.target}</dt><dd className="mt-1 font-medium text-mist">{formatPercent(target.targetPercentage)}</dd></div><div><dt className="text-muted">{copy.acceptable}</dt><dd className="mt-1 font-medium text-mist">{formatPercent(minimum)} – {formatPercent(maximum)}</dd></div><div className="sm:col-span-2"><dt className="text-muted">{copy.difference}</dt><dd className={`mt-1 font-medium ${target.state === 'WithinRange' ? 'text-success' : 'text-warning'}`}>{target.differencePercentage >= 0 ? '+' : ''}{formatPercent(target.differencePercentage)}</dd></div></dl></div>
    })}</div></>}
    {editing && <div className="mt-5 border-t border-white/10 pt-5"><div className="grid gap-3 sm:grid-cols-2"><label className="text-xs text-muted">{copy.strategyName}<input className="field mt-1" value={name} onChange={event => setName(event.target.value)} /></label><label className="text-xs text-muted">{copy.effectiveFrom}<input className="field mt-1" type="date" value={effectiveFrom} onChange={event => setEffectiveFrom(event.target.value)} /></label><label className="text-xs text-muted">{copy.endDate}<input className="field mt-1" type="date" value={effectiveTo} onChange={event => setEffectiveTo(event.target.value)} /></label><label className="text-xs text-muted">{copy.rationale}<input className="field mt-1" value={description} onChange={event => setDescription(event.target.value)} /></label></div><div className="mt-4 rounded-2xl border border-sky/20 bg-sky/10 p-4"><p className="eyebrow text-sky">{copy.portfolioValue}</p><p className="mt-1 text-2xl font-semibold text-white">{formatMoney(portfolioValue, currency, false, locale)}</p><p className="mt-1 text-xs leading-5 text-muted">{copy.portfolioValueHelp}</p></div><p className="mt-4 rounded-xl border border-sky/20 bg-sky/10 px-4 py-3 text-xs leading-5 text-sky">{copy.defaultHelp}</p><div className="mt-4 space-y-3">{targets.map((target, index) => {
      const current = currentAllocation(target.category)
      return <div className="grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-4 sm:grid-cols-2" key={`${target.category}-${index}`}><label className="text-xs text-muted sm:col-span-2">{copy.category}<select className="field mt-1" value={target.category} onChange={event => setTargets(items => items.map((item, itemIndex) => itemIndex === index ? { ...item, category: event.target.value } : item))}><option className="bg-panel" value="">—</option>{categoryOptions.map(category => <option className="bg-panel" key={category} value={category}>{assetKindLabel(locale, category)}</option>)}</select></label><dl className="grid grid-cols-2 gap-3 rounded-xl border border-white/10 bg-ink/20 p-3 text-xs sm:col-span-2"><div><dt className="text-muted">{copy.currentValue}</dt><dd className="mt-1 font-semibold text-mist">{current ? formatMoney(current.value, currency, false, locale) : '—'}</dd></div><div><dt className="text-muted">{copy.current}</dt><dd className="mt-1 font-semibold text-mist">{current ? formatPercent(current.percentage) : '—'}</dd></div></dl><label className="text-xs text-muted">{copy.targetInput}<input className="field mt-1" min="0" max="100" step="any" type="number" value={target.targetPercentage} onChange={event => setTargets(items => items.map((item, itemIndex) => itemIndex === index ? { ...item, targetPercentage: event.target.value } : item))} /></label><label className="text-xs text-muted">{copy.toleranceInput}<input className="field mt-1" min="0" max="100" step="any" type="number" value={target.tolerancePercentage} onChange={event => setTargets(items => items.map((item, itemIndex) => itemIndex === index ? { ...item, tolerancePercentage: event.target.value } : item))} /></label><p className="text-xs leading-5 text-muted sm:col-span-2">{copy.toleranceHelp}</p><button className="justify-self-start text-xs text-danger" type="button" onClick={() => setTargets(items => items.filter((_, itemIndex) => itemIndex !== index))}>× {copy.remove}</button></div>
    })}</div><div className="mt-4 flex flex-wrap gap-2"><button className="ghost-button px-3 py-2 text-xs" type="button" onClick={() => setTargets(items => [...items, { category: '', targetPercentage: '0', tolerancePercentage: '5' }])}>{copy.add}</button><button className="primary-button px-4 py-2 text-xs" type="button" disabled={!name.trim() || !targets.some(target => target.category.trim())} onClick={() => void save()}>{copy.save}</button><button className="ghost-button px-3 py-2 text-xs" type="button" onClick={() => setEditing(false)}>{copy.cancel}</button>{strategyId && <button className="ml-auto text-xs text-danger" type="button" onClick={() => void onDelete(strategyId).then(() => setEditing(false))}>{copy.delete}</button>}</div></div>}
  </article>
}
