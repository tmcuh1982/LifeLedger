...
import type { AssetCategory, LedgerItem, ScenarioData } from '../types'
...
function newDraft(resource: Resource, currency: string): Draft {
  ...
  if (resource === 'assets') return { ...base, kind: 'Cash', assetCategory: 'builtin:Cash', customCategory: '', currentValue: '', ticker: '', quantity: '0', capitalGainsTaxRate: '0', capitalGainsTaxCountryCode: '', expectedAnnualReturn: '0', volatility: '0', isLiquid: true }
...
function editDraft(resource: Resource, item: LedgerItem, currency: string): Draft {
  ...
  if (resource === 'assets') return { ...base, kind: value(item, 'kind'), assetCategory: item.customCategory ? `custom:${String(item.customCategory)}` : `builtin:${value(item, 'kind') || 'Other'}`, customCategory: value(item, 'customCategory'), currentValue: value(item, 'currentValue'), ticker: value(item, 'ticker'), quantity: value(item, 'quantity'), capitalGainsTaxRate: percent(item, 'capitalGainsTaxRate'), capitalGainsTaxCountryCode: value(item, 'capitalGainsTaxCountryCode'), expectedAnnualReturn: percent(item, 'expectedAnnualReturn'), volatility: percent(item, 'volatility'), isLiquid: checked(item, 'isLiquid', true) }
...
export function Planner({ data, assetCategories, currency, locale, onCreate, onUpdate, onDelete }: PlannerProps) {
  ...
  function setField(name: string, next: string | boolean) {
    setDraft((current) => {
      if (active?.resource === 'assets' && name === 'assetCategory' && typeof next === 'string') {
        ...
        const updated = { ...current, assetCategory: next, kind, customCategory }
